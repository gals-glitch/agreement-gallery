# QUICK CSV IMPORT V4 - FIXED: Add source_kind for investors

$ErrorActionPreference = "Stop"

Write-Host "Generating import SQL (V4 - With source_kind)..." -ForegroundColor Cyan

$importDir = "C:\Users\GalSamionov\Downloads"
$outputSql = "$PSScriptRoot\GENERATED_IMPORT_V4.sql"

$sqlLines = @()
$sqlLines += "-- QUICK IMPORT V4 - Generated $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
$sqlLines += "-- FIX: Added source_kind for investors to pass constraint check"
$sqlLines += "BEGIN;"
$sqlLines += ""
...
# Import investors WITH source_kind
$sqlLines += "-- STEP 2: Import Investors (WITH source_kind)"
$sqlLines += ""
...
foreach ($investor in $investors) {
    ...
    if ($name -and $name -ne "Totals") {
        ...
        # *** FIX: Set source_kind based on whether there's a party link ***
        $sourceKind = if ($intro) { "'DISTRIBUTOR'" } else { "'NONE'" }
        ...
        # *** FIX: Include source_kind in INSERT ***
        $sqlLines += "INSERT INTO investors (name, introduced_by_party_id, source_kind, notes)"
        $sqlLines += "VALUES ('$name', $partyIdVal, $sourceKind, $notesVal)"
        $sqlLines += "ON CONFLICT (name) DO UPDATE"
        $sqlLines += "SET introduced_by_party_id = COALESCE(EXCLUDED.introduced_by_party_id, investors.introduced_by_party_id),"
        $sqlLines += "    source_kind = COALESCE(EXCLUDED.source_kind, investors.source_kind),"
...
Write-Host "Investors: $investorCount (with source_kind)" -ForegroundColor Green
...
# Import contributions (auto-create investors if missing - WITH source_kind)
$sqlLines += "-- STEP 4: Import Contributions (auto-create investors with source_kind if missing)"
$sqlLines += ""
...
foreach ($contribution in $contributions) {
    ...
    if ($investorName -and $dealName -and $amount -gt 0) {
        # Auto-create investor if doesn't exist (with source_kind='NONE' for auto-created ones)
        $sqlLines += "INSERT INTO investors (name, source_kind) VALUES ('$investorName', 'NONE') ON CONFLICT (name) DO NOTHING;"
...
$sqlLines += "UNION ALL SELECT 'Investors (with party links)', COUNT(*) FROM investors WHERE introduced_by_party_id IS NOT NULL"
$sqlLines += "UNION ALL SELECT 'Investors (source=DISTRIBUTOR)', COUNT(*) FROM investors WHERE source_kind = 'DISTRIBUTOR'"
...
Write-Host "  Investors: $investorCount (with source_kind)"
Write-Host "  Agreements: $agreementCount"
Write-Host "  Contributions: $contributionCount"
Write-Host ""
Write-Host "FIX: Added source_kind='DISTRIBUTOR' for investors with party links" -ForegroundColor Yellow
Write-Host "FIX: Added source_kind='NONE' for auto-created investors from contributions" -ForegroundColor Yellow
Write-Host ""
Write-Host "Next: Copy $outputSql contents to Supabase SQL Editor and execute"
