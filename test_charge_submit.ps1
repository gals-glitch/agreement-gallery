# Smoke Test 3: Submit Charge (DRAFT → PENDING)
# This will auto-apply any available credits via creditsEngine
...
$uri = "https://qwgicrdcoqdketqhxbys.supabase.co/functions/v1/api-v1/charges/$chargeId/submit"

Write-Host "🚀 Submitting charge for approval..." -ForegroundColor Cyan
Write-Host "Charge ID: $chargeId" -ForegroundColor Gray
Write-Host "Expected: Status changes DRAFT → PENDING" -ForegroundColor Gray
Write-Host ""

try {
    $response = Invoke-RestMethod -Uri $uri -Method Post -Headers $headers

    Write-Host "✅ SUCCESS! Charge submitted:" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "  Credits Applied: `$$($response.credits_applied_amount)" -ForegroundColor White
    Write-Host "  Net Amount: `$$($response.net_amount)" -ForegroundColor White
