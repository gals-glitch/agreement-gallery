-- STEP 2: The robust merge_investors() function
...
BEGIN
  ...
  -- Optional: keep the "better" name/email on dst if missing
  UPDATE public.investors d
  SET name  = COALESCE(d.name,  s.name),
      email = COALESCE(d.email, s.email),
      phone = COALESCE(d.phone, s.phone),
      address = COALESCE(d.address, s.address),
      tax_id = COALESCE(d.tax_id, s.tax_id)
  FROM public.investors s
  WHERE d.id = dst_id AND s.id = src_id;
