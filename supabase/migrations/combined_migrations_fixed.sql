...
SELECT f.id, 'A', 120, 80, 24, 0,       3000000,  DATE '2025-01-01', true, 1 FROM f
UNION ALL
SELECT f.id, 'B', 180, 80, 24, 3000001, 6000000,  DATE '2025-01-01', true, 1 FROM f
UNION ALL
SELECT f.id, 'C', 180, 130,24, 6000001, NULL,     DATE '2025-01-01', true, 1 FROM f
...
BEGIN
  ...
  -- Try to change pricing mode (should FAIL)
  BEGIN
    UPDATE agreements
    SET selected_track = 'A'
    WHERE id = test_agreement_id;

    RAISE EXCEPTION '❌ TEST FAILED: Trigger did not block pricing change';
  EXCEPTION
    WHEN raise_exception THEN
      IF SQLERRM LIKE '%immutable%' THEN
        RAISE NOTICE '✅ TEST 1 PASSED: Trigger blocked pricing change';
      ELSE
        RAISE;
      END IF;
  END;

  -- Try to mark as SUPERSEDED (should SUCCEED)
  BEGIN
    UPDATE agreements
    SET status = 'SUPERSEDED', effective_to = '2025-12-31'
    WHERE id = test_agreement_id;

    RAISE NOTICE '✅ TEST 2 PASSED: Trigger allowed SUPERSEDED transition';
  EXCEPTION
    WHEN OTHERS THEN
      RAISE EXCEPTION '❌ TEST 2 FAILED: Trigger blocked SUPERSEDED transition: %', SQLERRM;
  END;
  ...
  RAISE NOTICE 'All tests passed! Immutability trigger fixed.';
END $$;
