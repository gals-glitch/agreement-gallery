-- ============================================================================
-- COMMISSION DATA IMPORT SCRIPT - FILLED WITH YOUR DATA (FIXED)
-- ============================================================================
-- FIXED: Handles duplicate investor names by keeping the last occurrence
-- IMPORTANT NOTES:
...
-- 3. Duplicate investor names are de-duplicated (last occurrence wins)
...
-- Show imported parties count
DO $$
BEGIN
    RAISE NOTICE 'Parties imported: %', (SELECT COUNT(*) FROM parties WHERE name IN (SELECT party_name FROM temp_parties));
END $$;
...
CREATE TEMP TABLE temp_investors_raw (
    row_num SERIAL,
    investor_name TEXT,
    party_name TEXT,
    email TEXT,
...
-- Insert ALL investor data (including duplicates)
INSERT INTO temp_investors_raw (investor_name, party_name, email, phone, notes) VALUES
...
-- De-duplicate: keep only the LAST occurrence of each investor_name
CREATE TEMP TABLE temp_investors AS
SELECT DISTINCT ON (investor_name)
    investor_name,
    party_name,
    email,
    phone,
    notes
FROM temp_investors_raw
ORDER BY investor_name, row_num DESC;  -- Last occurrence wins
...
BEGIN
    SELECT COUNT(*) INTO raw_count FROM temp_investors_raw;
    SELECT COUNT(*) INTO dedup_count FROM temp_investors;
    removed_count := raw_count - dedup_count;
    RAISE NOTICE 'Raw investor rows: %, De-duplicated: %, Duplicates removed: %', raw_count, dedup_count, removed_count;
END $$;

-- Now insert investors (no more duplicates!)
...
-- Show imported investors count
DO $$
BEGIN
    RAISE NOTICE 'Investors imported: %', (SELECT COUNT(*) FROM investors WHERE name IN (SELECT investor_name FROM temp_investors));
END $$;
...
SELECT
    ...
    ta.fund_id,
    ta.deal_id,
    ta.status::agreement_status as status,
...
-- Show imported agreements count
DO $$
BEGIN
    RAISE NOTICE 'Agreements imported: %', (SELECT COUNT(*) FROM agreements WHERE kind = 'distributor_commission' AND created_at >= (NOW() - INTERVAL '1 hour'));
END $$;
...
-- Verify agreement scope validation
SELECT
    '=== Agreement Scope Validation ===' as section,
    COUNT(*) as total_agreements,
    SUM(CASE WHEN deal_id IS NOT NULL THEN 1 ELSE 0 END) as deal_scoped,
    SUM(CASE WHEN fund_id IS NOT NULL THEN 1 ELSE 0 END) as fund_scoped
FROM agreements
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '⚠️  ⚠️  ⚠️  ACTION REQUIRED  ⚠️  ⚠️  ⚠️';
    RAISE NOTICE '';
    RAISE NOTICE '  All agreements are set to deal_id=1 as placeholder!';
    RAISE NOTICE '  Run UPDATE queries to set correct deal_id for each agreement.';
    RAISE NOTICE '';
    RAISE NOTICE '  Example:';
    RAISE NOTICE '  UPDATE agreements ';
    RAISE NOTICE '  SET deal_id = 2 ';
    RAISE NOTICE '  WHERE party_id = (SELECT id FROM parties WHERE name = ''Kuperman'');';
    RAISE NOTICE '';
END $$;
